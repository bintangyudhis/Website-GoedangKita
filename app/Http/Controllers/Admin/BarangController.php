...
class BarangController extends Controller
{
    ...
    public function getbarang(string $id): string
    {
        ...
        // json_encode bisa return false, jadi dipaksa jadi string supaya cocok dengan return type
    ...
    public function show(Request $request): ?JsonResponse
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) use ($roleId) {
                $barangNamaSlug = preg_replace(
                    '/[^A-Za-z0-9-]+/',
                    '_',
                    (string) $row->barang_nama
                );
                $barangNamaSlug = trim($barangNamaSlug ?? '');
    ...
    public function listbarang(Request $request): ?JsonResponse
    {
        if (! $request->ajax()) {
            return null;
        }
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) use ($request) {
                ...
                $satuanNamaSlug = preg_replace(
                    '/[^A-Za-z0-9-]+/',
                    '_',
                    (string) $row->satuan_nama
                );
                $satuanNamaSlug = trim($satuanNamaSlug ?? '');

                $jenisbarangNamaSlug = preg_replace(
                    '/[^A-Za-z0-9-]+/',
                    '_',
                    (string) $row->jenisbarang_nama
                );
                $jenisbarangNamaSlug = trim($jenisbarangNamaSlug ?? '');

                $array = [
                    'barang_kode'      => $row->barang_kode,
                    'barang_nama'      => $barangNamaSlug,
                    'satuan_nama'      => $satuanNamaSlug,
                    'jenisbarang_nama' => $jenisbarangNamaSlug,
                ];
    ...
    public function proses_tambah(Request $request): JsonResponse
    {
        ...
        /** @var string|null $rawNama */
        $rawNama = $request->input('nama');
        $namaForSlug = is_string($rawNama) ? $rawNama : '';
    ...
    public function proses_ubah(Request $request, BarangModel $barang): JsonResponse
    {
        /** @var string|null $rawNama */
        $rawNama = $request->input('nama');
        $namaForSlug = is_string($rawNama) ? $rawNama : '';
    ...
    public function proses_hapus(Request $request, BarangModel $barang): JsonResponse
    {
        // delete image
        Storage::delete('public/barang/' . $barang->barang_gambar);
