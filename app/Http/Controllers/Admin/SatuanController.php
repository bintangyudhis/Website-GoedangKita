...
class SatuanController extends Controller
{
    public function index(): View
    {
        ...
        $data['hakTambah'] = AksesModel::leftJoin('tbl_submenu', 'tbl_submenu.submenu_id', '=', 'tbl_akses.submenu_id')
            ->where([
                ...
                'tbl_submenu.submenu_judul' => 'Satuan',
    ...
    public function show(Request $request): ?JsonResponse
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) use ($roleId) {
                ...
                $nama = $row->satuan_nama;

                /** @var string|null $ket */
                $ket = $row->satuan_keterangan;

                $namaSlugSource = preg_replace(
                    '/[^A-Za-z0-9-]+/',
                    '_',
                    $nama ?? ''
                );
                $namaSlug = trim($namaSlugSource ?? '');

                $ketSlugSource = preg_replace(
                    '/[^A-Za-z0-9-]+/',
                    '_',
                    $ket ?? ''
                );
                $ketSlug = trim($ketSlugSource ?? '');

                $array = [
                    'satuan_id'        => $row->satuan_id,
                    'satuan_nama'      => $namaSlug,
                    'satuan_keterangan'=> $ketSlug,
                ];
    ...
    {
        /** @var string|null $rawSatuan */
        $rawSatuan = $request->input('satuan');
    ...
    public function proses_ubah(Request $request, SatuanModel $satuan): JsonResponse
    {
        /** @var string|null $rawSatuan */
        $rawSatuan = $request->input('satuan');
    ...
    public function proses_hapus(Request $request, SatuanModel $satuan): JsonResponse
    {
        // delete
        $satuan->delete();
